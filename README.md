...
| `codex`      | Codex CLI (レビュー用エージェント)       | `npm install -g @openai/codex`           |
...
> **Note:** `specflow-init` は `.specflow/config.env`、`.specflow/config.yaml`、`CLAUDE.md` を生成し、`.gitignore` に `.specflow/runs/` などのローカル生成物を追加します。`openspec/` ディレクトリは `/specflow` 初回実行時に OpenSpec CLI が自動作成します。`/specflow.setup` は既存の `CLAUDE.md` をインタラクティブに設定するコマンドです。
>
> **Config ownership boundary:** specflow は 2 つの設定ファイルを持ちます。`.specflow/config.yaml` は **shared workflow policy**（プロジェクト全体で共有する設定。例: `apply.subagent_dispatch.*`、`max_autofix_rounds` などのレビュー/autofix tuning）を保持し、リポジトリにコミットします。`.specflow/config.env` は **local runtime / operator preference**（マシンごとの個人設定。例: main/review agent 選択、ローカル CLI パス）を保持し、`.gitignore` で管理されます。`openspec/config.yaml` は OpenSpec 自身の設定領域であり、specflow 設定はそこに置かないでください — 旧バージョンから移行する場合は deprecation warning が出ますので、設定を `.specflow/config.yaml` に移してください。
...
- `.specflow/config.env` — local runtime / operator preference（gitignored）。main/review agent 選択や CLI パスなど、マシンごとの設定
- `.specflow/config.yaml` — shared workflow policy（committed）。`apply.subagent_dispatch.*` やレビュー/autofix tuning など、プロジェクト全体で共有するワークフロー設定。デフォルトでは何もコメントアウトされた状態で生成されるため、必要なキーだけを有効化する
- `CLAUDE.md` — Claude Code 用プロジェクト設定テンプレート
- `.gitignore` の specflow 用 ignore エントリ — `.specflow/runs/`、`.specflow/config.env`、Claude local settings
...
| `codex`      | Codex CLI (review agent)                   | `npm install -g @openai/codex`       |
