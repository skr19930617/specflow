...
- build で `dist/package/` に配布用 bundle を生成
...
curl コマンドを再実行すると全体が最新に更新される（idempotent）。手動インストールの場合は `npm run build && node dist/bin/specflow-install.js` を再実行する。スラッシュコマンドや review prompt の更新を反映するには、再 build と再 install が必要。
...
1. specflow をインストール: `curl ... | bash` または `npm run build && node dist/bin/specflow-install.js`
...
| コンポーネント | ファイル | 役割 |
|---------------|---------|------|
| State Machine | `dist/package/global/workflow/state-machine.json` | 状態・イベント・遷移の宣言的定義 (v2.0) |
