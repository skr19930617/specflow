...
> **Note:** `specflow-init` は `.specflow/config.env` と `CLAUDE.md` を生成し、`.gitignore` に `.specflow/runs/` などのローカル生成物を追加します。`openspec/` ディレクトリは `/specflow` 初回実行時に OpenSpec CLI が自動作成します。`/specflow.setup` は既存の `CLAUDE.md` をインタラクティブに設定するコマンドです。
...
- `.gitignore` の specflow 用 ignore エントリ — `.specflow/runs/`、Claude local settings
