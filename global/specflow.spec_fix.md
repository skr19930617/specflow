...
Set `LEDGER_FILE` = `FEATURE_DIR/review-ledger-spec.json`.

Attempt to Read `LEDGER_FILE`:

- **If file does not exist**: Set `REREVIEW_MODE = false`.
- **If file exists and valid JSON with `findings` array**: Set `REREVIEW_MODE = true`. Extract `PREVIOUS_FINDINGS` from ledger `findings` array (only findings where status is NOT "resolved" — i.e., status in ["open", "new", "accepted_risk", "ignored"]). Extract `MAX_FINDING_ID` from ledger (if present), or derive from `max(findings.map(f => extractNumber(f.id)))`, or 0 if empty.
- **If file exists but `findings` array is empty**: Set `REREVIEW_MODE = true`, `PREVIOUS_FINDINGS` = empty array, `MAX_FINDING_ID = 0`.
- **If file exists but JSON parse fails or required fields missing**: Set `REREVIEW_MODE = true`, `PREVIOUS_FINDINGS` = empty array, `MAX_FINDING_ID = 0`, `LEDGER_ERROR = true`. Display: `"⚠ review-ledger-spec.json が破損しています。空の前回 findings で re-review を実行します。"`.
- **If file exists, JSON valid, findings present, but `max_finding_id` absent**: Derive from findings. Log: `"⚠ max_finding_id が見つかりません。findings から導出しました。"`. Proceed as valid ledger.
...
```
<review_spec_rereview_prompt.md の内容>
...
2. Attempt to Read `LEDGER_FILE` (`FEATURE_DIR/review-ledger-spec.json`).

   - **If file does not exist**: Create a new ledger: `{ "feature_id": "<BRANCH from check-prerequisites>", "phase": "spec", "current_round": 0, "status": "all_resolved", "findings": [], "round_summaries": [] }`
   - **If file exists but JSON parse fails**: Rename the corrupt file to `review-ledger-spec.json.corrupt` via Bash (`mv`). Attempt to Read `review-ledger-spec.json.bak`. If bak succeeds, use it and display: `"⚠ review-ledger-spec.json が破損していました。バックアップから復旧しました（破損ファイルは .corrupt に退避）"`. If bak also fails, use `AskUserQuestion` to ask `"新規 ledger を作成しますか？ (既存データは失われます)"` with options "新規作成" / "中止". On "中止", stop the workflow. On "新規作成", create a fresh empty ledger: `{ "feature_id": "<BRANCH from check-prerequisites>", "phase": "spec", "current_round": 0, "status": "all_resolved", "findings": [], "round_summaries": [] }` and continue normal processing. This is NOT a "clean read" — do not create a backup from this empty ledger.
...
6. **Step 1 — Same match** (`file` + `category` + `severity` exact):
   ...
   - 1:1 → same. N:M → normalize titles (lowercase + whitespace collapse + trim) and match exact. Remaining → pair by index order. Leftover Codex findings → Step 2.
...
13. If the ledger was a "clean read" (not recovered from backup): Write the pre-update ledger content to `review-ledger-spec.json.bak` via Write tool.
14. Write the updated ledger JSON to `review-ledger-spec.json` via Write tool.

### Ledger Summary Display

15. Display before the findings table:
    ```
    Review Ledger (spec): Round {current_round} | Status: {status} | Findings: {new} new, {open} open, {resolved} resolved
    ```
...
## Step 2.7: Low Severity Auto-Apply

After the ledger update, check for low-severity findings that can be auto-applied:

1. Filter all findings in the updated ledger where `severity == "low"` AND `status in ["new", "open"]`.
2. For each low-severity finding:
   - Attempt to apply the fix using `suggested_resolution` (if present) or `detail` field directly to `FEATURE_SPEC`.
   - If the fix is straightforward and unambiguous, apply it.
   - Mark the finding as `status = "resolved"`, `latest_round = current_round` in the ledger.
3. If any low-severity findings were auto-applied:
   - Display: `"✅ Low severity findings を自動修正しました: {count} 件"` with a list of resolved IDs.
   - Write the updated ledger to `review-ledger-spec.json` (no backup needed — already backed up in Step 2.5).
   - **Do NOT trigger a re-review** for low-severity auto-fixes.
   - If any findings were auto-applied, regenerate `FEATURE_DIR/current-phase.md` using the same logic as Step 2.6 (to reflect the post-auto-apply ledger state).
4. If no low-severity findings qualify: skip silently.
