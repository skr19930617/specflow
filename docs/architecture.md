...
| Run-state JSON structure | `src/types/contracts.ts` (`CoreRunState`, `LocalRunState`, `RunState`) | Persisted run-state shape including phase, history, agents, and metadata | **Partially supported** — the type surface is split into `CoreRunState` (runtime-agnostic fields every external runtime persists) and `LocalRunState` (local-adapter-only fields such as `repo_path`/`worktree_path`). `RunState` is preserved as `CoreRunState & LocalRunState` for backwards compatibility. The compile-time drift guard at `src/tests/run-state-partition.test.ts` enforces disjointness and exhaustiveness. The matching JSON-schema-level core/local split is deferred to a follow-up proposal under the Deterministic Server Orchestrator epic; `specflow-run` remains the current local implementation of this contract surface. |
...
- **Core-adjacent modules**: `src/types/contracts.ts` — this module contains type definitions and runtime const objects (e.g., `AssetType`) with no Node.js or adapter imports. Both value and type imports from this module are allowed in core. The run-state type is explicitly partitioned into `CoreRunState` (core-contract fields) and `LocalRunState` (local-adapter fields) with `RunState` preserved as their intersection; core modules SHOULD depend on `CoreRunState` when they do not read local-adapter fields (see `src/core/advance.ts`, `suspend.ts`, `resume.ts`, and `_helpers.ts` for the `<T extends CoreRunState = RunState>` pattern). The compile-time drift guard at `src/tests/run-state-partition.test.ts` prevents regressions.
