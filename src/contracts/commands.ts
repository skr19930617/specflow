...
export const commandContracts: readonly CommandContract[] = [
  ...
  command("specflow.approve", "実装を承認し、Archive → コミット → Push → PR 作成", [], [
    hook(
      "Apply Acceptance",
  ...
  command(
    ...
    ["handoff:specflow.apply", "handoff:specflow.fix_design", "handoff:specflow.reject"],
    [
      hook(
  ...
  command(
    ...
    ["handoff:specflow.approve", "handoff:specflow.fix_apply", "handoff:specflow.reject"],
    [
      hook(
        "Apply Revision Loop",
  ...
  command("specflow.decompose", "specの複雑さを分析し、issue-linked specはGitHub sub-issueに分解、inline specは警告を表示", []),
  command("specflow.dashboard", "全featureのレビュー台帳を集計し、ダッシュボードとして表示・保存", []),
  command("specflow.setup", "CLAUDE.md をインタラクティブに設定（Tech Stack, Commands, Code Style）", []),
  command("specflow.license", "プロジェクト解析に基づいてライセンスファイルを生成", []),
  command("specflow.readme", "プロジェクト解析に基づいて OSS 風 README を生成・更新", []),
];
