...
function openspec(args: readonly string[]) {
  return tryExec(resolveCommand("SPECFLOW_OPENSPEC", "openspec"), args, process.cwd());
}
...
function cmdNext(changeId: string): never {
  ...
  const artifacts = Array.isArray(statusJson.artifacts) ? statusJson.artifacts : [];
  const ready = artifacts.filter((artifact) => artifact && typeof artifact === "object" && (artifact as { status?: unknown }).status === "ready");
  if (ready.length === 0) {
  ...
  printSchemaJson("design-artifact-next", {
    ...
    dependencies: Array.isArray(instructions.dependencies)
      ? instructions.dependencies.map((dependency) => {
          const value = dependency as { id?: unknown; path?: unknown; done?: unknown };
          return {
            id: value.id,
            path: value.path,
            done: value.done,
