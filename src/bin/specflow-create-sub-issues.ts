...
function parseInput(raw: string): CreateSubIssuesInput {
	...
	const validationErrors = validateSchemaValue(
	...
	return assertSchemaValue(
		"create-sub-issues-input",
		parsed as CreateSubIssuesInput,
		"stdin",
	);
}
...
function decompositionId(
	input: CreateSubIssuesInput,
	subFeature: CreateSubIssueInputItem,
): string {
...
function createIssue(
	input: CreateSubIssuesInput,
	subFeature: CreateSubIssueInputItem,
): CreateSubIssueCreated | CreateSubIssueFailed {
...
function isCreatedIssue(
	result: CreateSubIssueCreated | CreateSubIssueFailed,
): result is CreateSubIssueCreated {
	return "issue_number" in result;
}

function renderSummaryComment(
	input: CreateSubIssuesInput,
	created: readonly CreateSubIssueCreated[],
): string {
	const lines = [...created]
		.sort((left, right) => left.phase_number - right.phase_number)
		.map(
			(issue) =>
				`- **Phase ${issue.phase_number}**: #${issue.issue_number} — ${issue.title}`,
		);
	return [
