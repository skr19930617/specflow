...
function listRootMatches(cwd: string, patterns: readonly string[]): string[] {
  ...
  return entries
    .filter((entry) => patterns.some((pattern) => matchesGlobPattern(entry, pattern)))
    .sort();
}
...
function detectLicense(cwd: string): string | null {
  ...
  const head = readFileSync(resolve(cwd, file), "utf8").split("\n").slice(0, 5).join("\n");
...
function main(): void {
  ...
  const contributing = readTextIfExists(resolve(root, "CONTRIBUTING.md")) || null;
  ...
  if (existsSync(packageJsonPath)) {
    ...
    const packageJson = tryParseJson<Record<string, unknown>>(readFileSync(packageJsonPath, "utf8")) ?? {};
    description = typeof packageJson.description === "string" ? packageJson.description : null;
    scripts = typeof packageJson.scripts === "object" && packageJson.scripts ? (packageJson.scripts as Record<string, unknown>) : {};
    const deps = {
  ...
  if (existsSync(resolve(root, "build.gradle")) || existsSync(resolve(root, "pom.xml"))) {
    languages.push("Java");
  }
