...
function normalizeGhState(state: {
	...
	return {
		...state,
		labels: [...state.labels].sort((left, right) =>
			JSON.stringify(left).localeCompare(JSON.stringify(right)),
		),
		issues: [...state.issues].sort((left, right) =>
			JSON.stringify(left).localeCompare(JSON.stringify(right)),
		),
		comments: [...state.comments].sort((left, right) =>
			JSON.stringify(left).localeCompare(JSON.stringify(right)),
		),
	};
}
...
test("specflow-create-sub-issues matches archived success fixtures", () => {
	...
	try {
		...
		const nodeResult = runNodeCli(
			...
			tempRoot,
			prependPath(
				{ SPECFLOW_TEST_GH_STATE: nodeStub.statePath },
				nodeStub.stubDir,
			),
			input,
		);
		...
		assert.deepEqual(
			...
			readFixtureJson("create-sub-issues/success-output.json"),
		);
		assert.deepEqual(
...
test("specflow-create-sub-issues matches archived partial failure fixtures", () => {
	...
	try {
		...
		writeFileSync(
			nodeStub.statePath,
			`${JSON.stringify(failConfig, null, 2)}\n`,
			"utf8",
		);
		...
		assert.deepEqual(
			JSON.parse(nodeResult.stdout),
			readFixtureJson("create-sub-issues/partial-output.json"),
		);
		assert.deepEqual(
			normalizeGhState(readJson(nodeStub.statePath)),
			normalizeGhState(
