...
function normalizeFindings(input: unknown): ReviewFinding[] {
  ...
  return input.filter((item) => item && typeof item === "object") as ReviewFinding[];
}
...
export function readLedger(changeDir: string, config: LedgerConfig): LedgerReadResult {
  ...
  try {
    return {
      ledger: parseJson<ReviewLedger>(readFileSync(path, "utf8"), config.filename),
      status: "clean",
    };
  } catch {
  ...
  if (existsSync(backupPath)) {
    try {
      return {
        ledger: parseJson<ReviewLedger>(readFileSync(backupPath, "utf8"), `${config.filename}.bak`),
        status: "recovered",
      };
    } catch {
...
function findingMatches(left: ReviewFinding, right: ReviewFinding, exactSeverity: boolean): boolean {
...
function makeNewFinding(finding: ReviewFinding, round: number, seq: number, status: "new" | "open", relation: string, supersedes: string | null): ReviewFinding {
  return {
    ...finding,
...
export function validateLedger(ledger: ReviewLedger): { ledger: ReviewLedger; warnings: string[] } {
  ...
  const findings = normalizeFindings(ledger.findings).map((finding) => {
    ...
    if (severity === "high" && (status === "accepted_risk" || status === "ignored") && notes.replace(/\s/g, "").length === 0) {
...
export function matchFindings(ledger: ReviewLedger, codexFindings: readonly ReviewFinding[], currentRound: number): ReviewLedger {
  ...
  const others = existing.filter((finding) => !isActive(finding) && !isOverride(finding));
  ...
  for (const [index, finding] of codex.entries()) {
    const match = [...active, ...overrides].find((candidate) => {
      ...
      return id.length > 0 && !matchedExisting.has(id) && findingMatches(candidate, finding, true);
    });
    if (!match) {
      continue;
    }
  ...
  for (const [index, finding] of codex.entries()) {
    ...
    step2New.push(makeNewFinding(finding, currentRound, seq, "open", "reframed", findingId(match)));
    seq += 1;
  }
  ...
  for (const [index, finding] of codex.entries()) {
    ...
    step3New.push(makeNewFinding(finding, currentRound, seq, "new", "new", null));
    seq += 1;
  }
...
export function matchRereview(ledger: ReviewLedger, response: Record<string, unknown>, currentRound: number): ReviewLedger {
  const resolvedIds = normalizeReferenceIds(response.resolved_previous_findings);
  const stillOpenIds = normalizeReferenceIds(response.still_open_previous_findings);
  ...
  const missingIds = priorIds.filter((id) => !cleanResolved.includes(id) && !stillOpenIds.includes(id));
...
export function applyStillOpenSeverityOverrides(ledger: ReviewLedger, values: unknown): ReviewLedger {
  if (!Array.isArray(values)) {
    return ledger;
  }
...
export function computeSummary(ledger: ReviewLedger, currentRound: number): ReviewLedger {
  ...
  const summary: LedgerRoundSummary = {
    ...
    resolved: findings.filter((finding) => findingStatus(finding) === "resolved").length,
...
export function computeStatus(ledger: ReviewLedger): ReviewLedger {
  ...
  const hasOpenHigh = findings.some((finding) => {
    const severity = findingSeverity(finding);
    const status = findingStatus(finding);
    return severity === "high" && (status === "open" || status === "new" || status === "accepted_risk" || status === "ignored");
  });
  const allResolved = findings.every((finding) => findingStatus(finding) === "resolved");
  return {
    ...ledger,
    status: hasOpenHigh ? "has_open_high" : allResolved ? "all_resolved" : "in_progress",
  };
}
...
export function backupAndWriteLedger(changeDir: string, ledger: ReviewLedger, config: LedgerConfig, cleanRead: boolean): void {
  const path = ledgerPath(changeDir, config);
  const backupPath = ledgerBackupPath(changeDir, config);
...
export function ledgerCounts(ledger: ReviewLedger): LedgerCounts {
  ...
  return {
    total: findings.length,
    open: findings.filter((finding) => findingStatus(finding) === "open").length,
    new: findings.filter((finding) => findingStatus(finding) === "new").length,
    resolved: findings.filter((finding) => findingStatus(finding) === "resolved").length,
    overridden: findings.filter((finding) => isOverride(finding)).length,
  };
...
function countBySeverity(findings: readonly ReviewFinding[], severity: string): LedgerSeverityCounts {
  const scoped = findings.filter((finding) => findingSeverity(finding) === severity);
  return {
    open: scoped.filter((finding) => findingStatus(finding) === "open").length,
    new: scoped.filter((finding) => findingStatus(finding) === "new").length,
    resolved: scoped.filter((finding) => findingStatus(finding) === "resolved").length,
    overridden: scoped.filter((finding) => isOverride(finding)).length,
  };
}

export function ledgerBySeverity(ledger: ReviewLedger): Record<string, LedgerSeverityCounts> {
  const findings = normalizeFindings(ledger.findings);
  return {
...
export function openHighFindings(ledger: ReviewLedger): ReviewFinding[] {
  return normalizeFindings(ledger.findings).filter((finding) => {
    const status = findingStatus(finding);
    return findingSeverity(finding) === "high" && (status === "new" || status === "open");
  });
}
...
export function resolvedHighFindingTitles(ledger: ReviewLedger): string[] {
  return normalizeFindings(ledger.findings)
    .filter((finding) => findingSeverity(finding) === "high" && findingStatus(finding) === "resolved")
    .map((finding) => String(finding.title ?? ""));
}
...
export function acceptedRiskSummary(ledger: ReviewLedger): string {
  const values = normalizeFindings(ledger.findings)
    ...
    .map((finding) => `${String(finding.title ?? "")} (${findingStatus(finding)}, notes: "${String(finding.notes ?? "")}")`);
  return values.length === 0 ? "none" : values.join("\n");
}

export function reemergedFindingTitle(newlyResolved: readonly string[], unresolvedHigh: readonly string[]): string | null {
