...
import { type DiffSummary, type ReviewFinding, type ReviewLedger } from "../types/contracts.js";
...
export function readReviewConfig(projectRoot: string): ReviewConfig {
  ...
  return {
    diffWarnThreshold: Number.isInteger(diffWarnThreshold) ? diffWarnThreshold : 1000,
    maxAutofixRounds:
      Number.isInteger(maxAutofixRounds) && maxAutofixRounds >= 1 && maxAutofixRounds <= 10
        ? maxAutofixRounds
        : 4,
  };
}

export function validateChangeDir(projectRoot: string, changeId: string): string {
...
export function resolvePromptPath(runtimeRoot: string, promptFile: string): string {
  const installed = resolve(process.env.HOME ?? "", ".config/specflow/global/prompts", promptFile);
...
export function callCodex<T>(cwd: string, promptContent: string): CodexCallResult<T> {
  const codex = resolveCommand("SPECFLOW_CODEX", "codex");
  const outputPath = join(tmpdir(), `specflow-codex-output-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.json`);
  const result = tryExec(codex, ["exec", "--full-auto", "--ephemeral", "-o", outputPath, promptContent], cwd);
  let rawOutput = "";
...
export function renderCurrentPhase(changeDir: string, ledger: ReviewLedger, kind: "apply" | "design", cwd: string): void {
  ...
  const openHighStr =
    openHigh.length > 0
      ...
      : "0 件";
  const acceptedRisks = (ledger.findings ?? [])
    .filter((finding) => {
      const status = String(finding.status ?? "");
      return status === "accepted_risk" || status === "ignored";
    })
    .map((finding) => `${String(finding.title ?? "")} (${String(finding.status ?? "")}, notes: "${String(finding.notes ?? "")}")`)
    .join("\n") || "none";
  const nextAction =
    kind === "apply"
      ? openHigh.length > 0
...
export function warnValidationRevert(ids: readonly string[]): void {
  ...
  process.stderr.write(`[ledger] WARNING: Reverted high-severity findings with empty notes to 'open': ${ids.join(", ")}\n`);
}
...
export function resolvedHighTitles(ledger: ReviewLedger): string[] {
  return (ledger.findings ?? [])
    .filter((finding) => String(finding.severity ?? "") === "high" && String(finding.status ?? "") === "resolved")
    .map((finding) => String(finding.title ?? ""));
}

export function newHighCount(current: readonly string[], previous: readonly string[]): number {
  return current.filter((value) => !previous.includes(value)).length;
}

export function reemergedTitle(newlyResolved: readonly string[], unresolved: readonly string[]): string | null {
...
export function diffWarningSummary(summary: DiffSummary, threshold: number): DiffSummary {
  return {
    ...summary,
