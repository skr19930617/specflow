...
export function git(args: readonly string[], cwd: string, env: NodeJS.ProcessEnv = process.env): string {
  return exec("git", args, cwd, env).trim();
}

export function tryGit(args: readonly string[], cwd: string, env: NodeJS.ProcessEnv = process.env) {
...
export function recentChanges(cwd: string, baseBranch = process.env.BASE_BRANCH || "main"): string {
  const base = tryGit(["merge-base", "HEAD", baseBranch], cwd);
  if (base.status !== 0 || !base.stdout.trim()) {
    return "  - (no commits yet)";
  }
  const log = tryGit(["log", "--oneline", "-5", `${base.stdout.trim()}..HEAD`], cwd);
