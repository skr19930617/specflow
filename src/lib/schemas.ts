...
type SchemaValidator = (value: unknown, path: string, errors: ValidationErrors) => void;
...
function expectRecord(value: unknown, path: string, errors: ValidationErrors): Record<string, unknown> | null {
  if (isRecord(value)) {
    return value;
  }
...
function optional(value: unknown, validator: SchemaValidator, path: string, errors: ValidationErrors): void {
  if (value === undefined) {
    return;
  }
...
function stringValidator(value: unknown, path: string, errors: ValidationErrors): void {
  if (typeof value !== "string") {
    push(errors, path, "must be a string.");
  }
}

function nonEmptyStringValidator(value: unknown, path: string, errors: ValidationErrors): void {
  if (typeof value !== "string" || value.length === 0) {
    push(errors, path, "must be a non-empty string.");
  }
}

function numberValidator(value: unknown, path: string, errors: ValidationErrors): void {
  if (typeof value !== "number" || !Number.isFinite(value)) {
    push(errors, path, "must be a finite number.");
  }
}

function booleanValidator(value: unknown, path: string, errors: ValidationErrors): void {
  if (typeof value !== "boolean") {
    push(errors, path, "must be a boolean.");
  }
}

function nullOrStringValidator(value: unknown, path: string, errors: ValidationErrors): void {
  if (value !== null && typeof value !== "string") {
    push(errors, path, "must be a string or null.");
  }
}

function nullOrObjectValidator(value: unknown, path: string, errors: ValidationErrors): void {
  if (value !== null && !isRecord(value)) {
    push(errors, path, "must be an object or null.");
  }
}

function stringArrayValidator(value: unknown, path: string, errors: ValidationErrors): void {
...
function objectArrayValidator(value: unknown, path: string, errors: ValidationErrors): void {
  if (!Array.isArray(value)) {
    push(errors, path, "must be an array.");
    return;
  }
...
function issueMetadataValidator(value: unknown, path: string, errors: ValidationErrors): void {
...
function diffSummaryValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  if (!Array.isArray(record.excluded)) {
    ...
    for (let index = 0; index < record.excluded.length; index += 1) {
      const item = expectRecord(record.excluded[index], `${path}.excluded[${index}]`, errors);
      if (!item) {
        continue;
      }
      stringValidator(item.file, `${path}.excluded[${index}].file`, errors);
      stringValidator(item.reason, `${path}.excluded[${index}].reason`, errors);
      optional(item.new_path, stringValidator, `${path}.excluded[${index}].new_path`, errors);
      optional(item.pattern, stringValidator, `${path}.excluded[${index}].pattern`, errors);
    }
  }
  ...
  optional(record.diff_warning, booleanValidator, `${path}.diff_warning`, errors);
}

function designArtifactNextValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  const status = record.status;
  if (status !== "ready" && status !== "complete" && status !== "blocked" && status !== "error") {
    push(errors, `${path}.status`, "must be ready, complete, blocked, or error.");
  }
  ...
  optional(record.dependencies, objectArrayValidator, `${path}.dependencies`, errors);
  optional(record.blocked, stringArrayValidator, `${path}.blocked`, errors);
  optional(record.error, stringValidator, `${path}.error`, errors);
}

function designArtifactValidateValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
...
function analyzeProjectValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  ...
  nullOrStringValidator(record.package_manager, `${path}.package_manager`, errors);
  ...
  nullOrStringValidator(record.existing_readme, `${path}.existing_readme`, errors);
...
function initProjectValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  ...
  optional(record.review_agent, stringValidator, `${path}.review_agent`, errors);
  optional(record.track_claude_dir, booleanValidator, `${path}.track_claude_dir`, errors);
  optional(record.openspec_initialized, booleanValidator, `${path}.openspec_initialized`, errors);
  ...
  stringArrayValidator(record.installed_commands, `${path}.installed_commands`, errors);
  stringArrayValidator(record.warnings, `${path}.warnings`, errors);
}

function reviewFindingValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  ...
  optional(record.origin_round, numberValidator, `${path}.origin_round`, errors);
  optional(record.latest_round, numberValidator, `${path}.latest_round`, errors);
  optional(record.resolved_round, numberValidator, `${path}.resolved_round`, errors);
}

function reviewPayloadValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  ...
  if (!Array.isArray(record.findings)) {
    ...
    for (let index = 0; index < record.findings.length; index += 1) {
      reviewFindingValidator(record.findings[index], `${path}.findings[${index}]`, errors);
    }
  }
...
function ledgerCountsValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
...
function ledgerSeverityCountsValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
...
function ledgerRoundSummaryValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  ...
  numberValidator(record.resolved, `${path}.resolved`, errors);
  numberValidator(record.overridden, `${path}.overridden`, errors);
  const bySeverity = expectRecord(record.by_severity, `${path}.by_severity`, errors);
  if (!bySeverity) {
    return;
  }
...
function ledgerSnapshotValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  ...
  if (bySeverity) {
    for (const [key, valueItem] of Object.entries(bySeverity)) {
      ledgerSeverityCountsValidator(valueItem, `${path}.by_severity.${key}`, errors);
    }
  }
  if (!Array.isArray(record.round_summaries)) {
    ...
    for (let index = 0; index < record.round_summaries.length; index += 1) {
      ledgerRoundSummaryValidator(record.round_summaries[index], `${path}.round_summaries[${index}]`, errors);
    }
  }
}

function handoffSummaryValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
...
function reviewDiffSummaryValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
...
function rereviewClassificationValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
...
function reviewResultValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  ...
  if (record.autofix !== null && record.autofix !== undefined) {
    ...
    if (autofix) {
      numberValidator(autofix.total_rounds, `${path}.autofix.total_rounds`, errors);
      ...
      if (!Array.isArray(autofix.divergence_warnings)) {
        push(errors, `${path}.autofix.divergence_warnings`, "must be an array.");
      }
    }
  }
  ...
  optional(record.diff_summary, reviewDiffSummaryValidator, `${path}.diff_summary`, errors);
  optional(record.diff_total_lines, numberValidator, `${path}.diff_total_lines`, errors);
  optional(record.warning, stringValidator, `${path}.warning`, errors);
  optional(record.ledger_recovery, stringValidator, `${path}.ledger_recovery`, errors);
  if (record.rereview_classification !== null && record.rereview_classification !== undefined) {
    rereviewClassificationValidator(record.rereview_classification, `${path}.rereview_classification`, errors);
  }
...
function runHistoryEntryValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
...
function runAgentsValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
...
function runStateValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  ...
  nullOrStringValidator(record.last_summary_path, `${path}.last_summary_path`, errors);
  ...
  if (!Array.isArray(record.history)) {
    ...
    for (let index = 0; index < record.history.length; index += 1) {
      runHistoryEntryValidator(record.history[index], `${path}.history[${index}]`, errors);
    }
  }
  if (record.run_kind !== undefined && record.run_kind !== "change" && record.run_kind !== "synthetic") {
    push(errors, `${path}.run_kind`, "must be change or synthetic.");
  }
}

function createSubIssueInputItemValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  ...
  stringArrayValidator(record.acceptance_criteria, `${path}.acceptance_criteria`, errors);
  numberValidator(record.phase_total, `${path}.phase_total`, errors);
}

function createSubIssuesInputValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  numberValidator(record.parent_issue_number, `${path}.parent_issue_number`, errors);
  nonEmptyStringValidator(record.repo, `${path}.repo`, errors);
  nonEmptyStringValidator(record.run_timestamp, `${path}.run_timestamp`, errors);
  if (!Array.isArray(record.sub_features) || record.sub_features.length === 0) {
    ...
    for (let index = 0; index < record.sub_features.length; index += 1) {
      createSubIssueInputItemValidator(record.sub_features[index], `${path}.sub_features[${index}]`, errors);
    }
  }
  optional(record.skip_comment, booleanValidator, `${path}.skip_comment`, errors);
}

function createSubIssueCreatedValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
...
function createSubIssueFailedValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
...
function createSubIssuesResultValidator(value: unknown, path: string, errors: ValidationErrors): void {
  const record = expectRecord(value, path, errors);
  if (!record) {
    return;
  }
  if (!Array.isArray(record.created)) {
    ...
    for (let index = 0; index < record.created.length; index += 1) {
      createSubIssueCreatedValidator(record.created[index], `${path}.created[${index}]`, errors);
    }
  }
  if (!Array.isArray(record.failed)) {
    ...
    for (let index = 0; index < record.failed.length; index += 1) {
      createSubIssueFailedValidator(record.failed[index], `${path}.failed[${index}]`, errors);
    }
  }
  booleanValidator(record.summary_comment_posted, `${path}.summary_comment_posted`, errors);
  numberValidator(record.parent_issue_number, `${path}.parent_issue_number`, errors);
}
...
export function validateSchemaValue(schemaId: SchemaId, value: unknown): readonly string[] {
  const errors: string[] = [];
  schemaValidators[schemaId](value, "$", errors);
  return errors;
}

export function assertSchemaValue<T>(schemaId: SchemaId, value: T, label = "payload"): T {
  const errors = validateSchemaValue(schemaId, value);
  if (errors.length > 0) {
    throw new Error(`${label} does not satisfy schema '${schemaId}': ${errors.join(" ")}`);
  }
  return value;
}

export function parseSchemaJson<T>(schemaId: SchemaId, raw: string, label = "payload"): T {
  let parsed: unknown;
  try {
