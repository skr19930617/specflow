...
The orchestrator SHALL manage the full ledger lifecycle: read (with corruption recovery), validate, increment round, match findings, compute summary, compute status, backup, and write.

#### Scenario: Ledger creation on first review
- **WHEN** `review-ledger.json` does not exist and `review` subcommand runs
...
- **WHEN** `review-ledger.json` exists but JSON parse fails and `review-ledger.json.bak` exists and is valid
...
- **WHEN** `review-ledger.json` is corrupt and no valid backup exists
